...
class Emulator:
    ...
    def __init__(self, program, address=0):
        ...
        stack_begin = 0xb000
        stack_end = stack_begin - len(stack)
        bus = Bus().add_device(ram, (address, address + len(ram))).add_device(stack, (stack_begin, stack_end))
        self.cpu = CPU(bus)
    ...
    def run(self, limit=0):
        ...
        while self.cpu.fetch():
            print(f'[{count}] {self.cpu.pc.value:016X}: ({self.cpu.instruction.value:08X}) {get_asm(self.cpu.instruction, use_symbol=True, pc=self.cpu.pc.value)}')
